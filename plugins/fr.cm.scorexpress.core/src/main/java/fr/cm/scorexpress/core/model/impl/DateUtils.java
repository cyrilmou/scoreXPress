...
public class DateUtils {
	...
	public static void add(final Date2 t,
			final Date d) {
		if (d != null) {
	...
	public static void remove(final Date2 t,
			final Date d) {
		if (d != null) {
			t.setTime(t.getTime() - d.getTime());
			if (d.getTime() != 0) {
	...
	public static void setTime(final Date2 d1,
			final Date2 d2) {
	...
	public static Date getHeure(final String heure) {
		...
		try {
			if (!negatif) {
				...
				final long offset = Calendar.getInstance().getTimeZone()
						.getOffset(d.getTime());
				if (d.getTime() > 0) {
		...
		try {
			...
			final long milliseconde = new Double(val * 24 * 3600000)
					.longValue();
	...
	public static String getHeureStr(Date date,
		...
		try {
			...
			if (date.getTime() < 0) {
				...
				date = DateFactory
						.createDate(nbHour * 3600000 - date.getTime());
			} else if (!hidePlus) {
				signe = "+";
			}
	...
	public static String getMinutesStr(final Date date,
			final boolean hidePlus) {
		if (sdfPerso != null) {
			return sdfPerso.format(date);
		}
	...
	public static void upTime(final Date2 date2,
			final Date d) {
		add(date2, d);
	}

	public static void downTime(final Date2 date2,
			final Date d) {
		remove(date2, d);
	}

	@SuppressWarnings({ "TypeMayBeWeakened" })
	public static boolean equalsDate(final Date2 temps,
			final Date2 temps1) {
