...
public class info {
    ...
    public static void calculPenaliteBalisesManquees(final ObjResultat resultat, final ObjUserChronos userChrono,
        ...
        for (final Balise balise : balises) {
            ...
            if (isNotValidate) {
                if (TYPE_OBLIGATOIRE.equalsIgnoreCase(balise.getType()) ||
                    ...
                    appendDataList(resultat, balise, sup, VAR_RESULTAT_BALISESMANQUEES);
                }
            } else {
                if (TYPE_PENALITY.equalsIgnoreCase(balise.getType())) {
                    upTime(penalite, balise.getPenalite());
                    upTime(resultat.getPenaliteAutre(), balise.getPenalite());
                    final Object baliseManqueRes = resultat.getInfo(VAR_RESULTAT_BALISESMANQUEES);
                    if (baliseManqueRes != null) {
                        resultat.setInfo(VAR_RESULTAT_BALISESMANQUEES, baliseManqueRes + ",+" + balise.getNum());
                    } else {
                        resultat.setInfo(VAR_RESULTAT_BALISESMANQUEES, '+' + balise.getNum());
                    }
                } else {
        ...
        final StringBuilder baliseOk = new StringBuilder();
        for (final ObjChrono chrono : userChrono.getChronos()) {
            if (!chrono.isNull() && etape.containBalise(chrono.getNumBalise())) {
                baliseOk.append(baliseOk.length() > 0 ? ',' : "").append(chrono.getNumBalise());
            }
        }
        if (baliseOk.length() > 0) {
            resultat.setInfo(VAR_RESULTAT_BALISES_OK, '[' + baliseOk.toString() + ']');
        }
    ...
    private static void appendDataList(final ObjResultat resultat, final Balise balise, final String sup,
                                       String attribut) {
        final Object baliseManqueRes = resultat.getInfo(attribut);
        if (baliseManqueRes != null) {
            resultat.setInfo(attribut, baliseManqueRes + "," + balise.getNum() + sup);
        } else {
            resultat.setInfo(attribut, balise.getNum() + sup);
        }
    }
