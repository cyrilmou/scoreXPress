        ...
        try {
            ...
            for (int nrLigneCSV = 0; (ligne = dis.readLine()) != null; nrLigneCSV++) {
                if (nrLigneCSV > 0) { // Titre
                    ...
                    int                        arg         = 0;
                    final String               newLine     = previousLine + ligne;
                    if (previousLine.length() > 0) {
                        System.err.println("Catching bad format:\n" + newLine);
                    }
                    ligne = newLine;
                    previousLine = "";
                    try {
                        for (; hasMore(ligne); arg++) {
                            ...
                            if (arg == 0) {
                                ...
                                if (time.getTime() < 0) {
                                    throw new RuntimeException(
                                            "Negative time for num" + userChronos.getDossard() + ": " + time);
                                } else if (!time.isNull()) {
                        ...
                        if (arg > 40) {
                            ...
                            throw new BadSportIdentFormat(null, nrLigneCSV, "Not enough information: " + arg,
                                                          userChronos.getDossard());
                        }
                    } catch (Throwable e) {
                        if (arg < 40) {
                            previousLine = newLine;
                        }
                        new BadSportIdentFormat(e, nrLigneCSV, "", userChronos.getDossard()).printStackTrace();
                    }
                } else {
                    if (config != null) {
...
class BadSportIdentFormat extends Exception {

    public BadSportIdentFormat(final Throwable e, final int line, final String cause, final String dossard) {
        super(cause + " dossard=" + dossard + " ligne=" + line, e);
    }
}
